...
def recomendar_produtos(usuario_id: int, preferencias: Preferencias) -> List[Produto]:
    ...
    produtos_recomendados = [produto for produto_id in historico_de_compras[usuario_id]
                             for produto in produtos if produto.id == produto_id]
    ...
    produtos_recomendados = [p for p in produtos_recomendados if any(
        tag in preferencias.tags for tag in p.tags)]  # Preferencias de tags
